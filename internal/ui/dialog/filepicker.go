...
func NewFilePicker(com *common.Common) *FilePicker {
	f := new(FilePicker)
	f.com = com
...
// Update updates the [FilePicker] dialog based on the given message.
func (f *FilePicker) Update(msg tea.Msg) tea.Msg {
	var cmds []tea.Cmd
	switch msg := msg.(type) {
	...
	return CmdMsg{tea.Batch(cmds...)}
}

// View renders the [FilePicker] dialog as a string.
func (f *FilePicker) View() string {
	...
	return HeaderInputListHelpView(t, f.width, filesHeight, header, imgPreview, files, f.help.View(f))
}
